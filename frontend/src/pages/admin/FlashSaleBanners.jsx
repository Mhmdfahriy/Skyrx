...
export default function AdminFlashSaleBanners() {
  ...
  const loadBanners = async () => {
    try {
      const response = await api.get('/admin/flash-sale-banners');
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.title || !formData.subtitle) {
      alert('Title dan Subtitle harus diisi');
    ...
    // Tambahkan image_base64 jika ada
    ...
    try {
      if (editMode) {
        await api.post(`/admin/flash-sale-banners/${currentBanner.id}`, payload);
      ...
      const errorMsg = error.response?.data?.message || 'Gagal menyimpan banner';
      alert(errorMsg);
    } finally {
      setSubmitLoading(false);
    }
      ...
      <div className="container mx-auto max-w-6xl">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-gray-400">Kelola banner promosi Anda</p>
          </div>
          <button
            onClick={() => {
              resetForm();
            ...
            <div className="bg-gray-800 rounded-xl p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto border border-gray-700">
              ...
              <form onSubmit={handleSubmit}>
                <div className="space-y-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium mb-2 text-gray-300">
                      Gambar Banner {!editMode && <span className="text-red-500">*</span>}
                    </label>
                    <input
                      type="file"
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium mb-2 text-gray-300">Tanggal Mulai</label>
                      <input
                        type="date"
                    ...
                    <div>
                      <label className="block text-sm font-medium mb-2 text-gray-300">Tanggal Selesai</label>
                      <input
                        type="date"
